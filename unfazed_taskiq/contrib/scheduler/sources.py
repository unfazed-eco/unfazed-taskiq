...
class TortoiseScheduleSource(ScheduleSource):
    def __init__(
        ...
        if alias not in unfazed_settings.DATABASE.connections:
            raise RuntimeError(f"Database connection {alias} not found")

        database_settings = unfazed_settings.DATABASE.connections[alias]

        if database_settings.driver != "unfazed.db.tortoise.Driver":
            raise RuntimeError(
                f"Database driver {database_settings.driver} not supported"
            )
    ...
    async def get_schedules(self) -> t.List["ScheduledTask"]:
        ...
        schedules = await m.PeriodicTask.filter(enabled=1).using_db(self.alias)
