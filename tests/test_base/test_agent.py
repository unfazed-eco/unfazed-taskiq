...
class TestAgentModule:
    ...
    def test_get_agent_returns_cached_agent_when_ready(self):
    ...
    def test_get_agent_initializes_agent_when_not_ready(self):
        ...
        with patch.dict(os.environ, {"UNFAZED_SETTINGS_MODULE": "test_settings"}):
            with patch('unfazed_taskiq.agent.import_setting') as mock_import_setting:
                mock_import_setting.return_value = {"UNFAZED_TASKIQ_SETTINGS": mock_settings}
                ...
                with patch('unfazed_taskiq.agent._agent', None):
                    with patch('unfazed_taskiq.agent.agent') as mock_agent:
                        ...
                        assert isinstance(mock_agent.setup.call_args[0][0], UnfazedTaskiqSettings)

    def test_get_agent_handles_import_setting_error(self):
        ...
        with patch.dict(os.environ, {"UNFAZED_SETTINGS_MODULE": "test_settings"}):
            with patch('unfazed_taskiq.agent.import_setting') as mock_import_setting:
                ...
                with patch('unfazed_taskiq.agent._agent', None):
                    from unfazed_taskiq.agent import get_agent
                    # Act & Assert
                    with pytest.raises(ImportError, match="Failed to import settings module"):
                        get_agent()

    def test_get_agent_handles_invalid_settings(self):
        ...
        with patch.dict(os.environ, {"UNFAZED_SETTINGS_MODULE": "test_settings"}):
            with patch('unfazed_taskiq.agent.import_setting') as mock_import_setting:
                mock_import_setting.return_value = {"UNFAZED_TASKIQ_SETTINGS": invalid_settings}
    ...
    def test_get_agent_caches_agent_instance(self):
    ...
    def test_global_variables_are_correct_types(self):
    ...
    def test_get_agent_with_different_settings_modules(self):
                ...
                with patch('unfazed_taskiq.agent.import_setting') as mock_import_setting:
                    ...
                    with patch('unfazed_taskiq.agent._agent', None):
                        with patch('unfazed_taskiq.agent.agent') as mock_agent:
                            ...
                            # Assert
                            assert result is mock_agent
                            mock_import_setting.assert_called_with("UNFAZED_SETTINGS_MODULE")