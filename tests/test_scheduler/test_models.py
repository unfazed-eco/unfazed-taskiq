...
class TestPeriodicTaskModel(object):
    async def test_periodic_task_model(self, test_scheduler_sample_data):
        ...
        sample_task_kwargs = {item["task_kwargs"] for item in test_scheduler_sample_data}
        assert db_task_kwargs == sample_task_kwargs

        db_task_schedule_aliases = {item.schedule_alias for item in db_data}
        sample_task_schedule_aliases = {item["schedule_alias"] for item in test_scheduler_sample_data}
        assert db_task_schedule_aliases == sample_task_schedule_aliases

    async def test_periodic_task_model_to_taskiq_schedule_task(self, test_scheduler_sample_data):
        assert test_scheduler_sample_data is not None
        assert len(test_scheduler_sample_data) == 4

        for item in test_scheduler_sample_data:
            pt_objs = await PeriodicTask.filter(task_name=item["task_name"]).first()
            assert pt_objs is not None
            schedule_data = pt_objs.to_taskiq_schedule_task()
...
class TestPeriodicTaskSerializer(object):
    async def test_periodic_task_serializer(self, test_scheduler_sample_data):
        assert test_scheduler_sample_data is not None
        assert len(test_scheduler_sample_data) == 4

        result = await PeriodicTaskSerializer.list(PeriodicTask.filter(), page=1, size=2)
