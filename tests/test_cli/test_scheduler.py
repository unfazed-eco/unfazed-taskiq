...
class TestSchedulerCMD(object):
    def test_scheduler_cmd_init(self):
    ...
    def test_exec_success_with_scheduler_alias(self):
    ...
    def test_exec_success_without_scheduler_alias(self):
...
class TestSchedulerArgs(object):
    def test_scheduler_event_args_init(self):
        ...
        assert args.configure_logging == True
        assert args.fs_discover == False
        assert args.tasks_pattern == ("**/tasks.py",)
        assert args.skip_first_run == False
        assert args.update_interval == None
        assert args.scheduler_alias == ()

    def test_scheduler_event_args_with_all_params(self):
        ...
        assert args.configure_logging == False
        assert args.fs_discover == True
        assert args.tasks_pattern == ("**/tasks.py", "**/jobs.py")
        assert args.skip_first_run == True
        assert args.update_interval == 30
        assert args.scheduler_alias == ("default", "worker")

    def test_from_cli_minimal_args(self):
        """Test from_cli with minimal arguments."""
        from unfazed_taskiq.cli.scheduler.args import SchedulerEventArgs

        args = ["test.scheduler", "test.module"]
        result = SchedulerEventArgs.from_cli(args)

        assert result.scheduler == "test.scheduler"
        assert result.modules == ["test.module"]
        assert result.log_level == "INFO"
        assert result.configure_logging == True
        assert result.fs_discover == False
        assert result.tasks_pattern == ["**/tasks.py"]
        assert result.skip_first_run == False
        assert result.update_interval == None
    ...
    def test_from_cli_with_all_options(self):
        ...
        assert result.configure_logging == False
        assert result.fs_discover == True
        assert result.tasks_pattern == ["**/tasks.py", "**/jobs.py"]
        assert result.skip_first_run == True
        assert result.update_interval == 60
        assert result.scheduler_alias == ["default", "worker"]

    def test_from_cli_with_short_options(self):
    ...
    def test_from_cli_with_no_modules(self):
    ...
    def test_from_cli_with_multiple_tasks_patterns(self):
    ...
    def test_from_cli_with_single_tasks_pattern(self):
    ...
    def test_dataclass_replace(self):
        ...
        assert updated.configure_logging == True
        assert updated.fs_discover == False
        assert updated.tasks_pattern == ["**/tasks.py"]
        assert updated.skip_first_run == False
        assert updated.update_interval == None
        ...
        assert updated2.configure_logging == False
        assert updated2.fs_discover == True
        assert updated2.tasks_pattern == ["**/custom_tasks.py"]
        assert updated2.skip_first_run == True
    ...
    def test_inheritance_from_scheduler_args(self):
