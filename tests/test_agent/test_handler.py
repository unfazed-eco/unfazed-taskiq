...
def handler_module(monkeypatch):
    ...
    def fake_setup(setup_alias: str, config):
...
class TestAgentHandler:
    def _make_handler(self, module, monkeypatch):
    ...
    def test_module_singletons(self, handler_module):
    ...
    def test_init_triggers_check_ready(self, handler_module):
    ...
    def test_register_and_duplicate(self, handler_module, monkeypatch):
    ...
    def test_reset(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_setup_requires_env(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        monkeypatch.delenv("UNFAZED_SETTINGS_MODULE", raising=False)
        with pytest.raises(ValueError, match="is not set"):
            handler.setup()

    def test_setup_import_error(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_setup_invalid_config(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        monkeypatch.setenv("UNFAZED_SETTINGS_MODULE", "module.path")
        monkeypatch.setattr(
            handler_module,
    ...
    def test_setup_success(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        monkeypatch.setenv("UNFAZED_SETTINGS_MODULE", "module.path")
        config = {
    ...
    def test_check_ready_paths(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        with (
    ...
    def test_get_agent_behaviour(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_get_agent_triggers_ready(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        handler._ready = False
    ...
    def test_scheduler_property(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_scheduler_triggers_ready(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_broker_property(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    def test_broker_triggers_ready(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    @pytest.mark.asyncio
    async def test_startup_iterates_agents(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
    ...
    @pytest.mark.asyncio
    async def test_shutdown_iterates_agents(self, handler_module, monkeypatch):
        handler = self._make_handler(handler_module, monkeypatch)
        first = SimpleNamespace(startup=AsyncMock(), shutdown=AsyncMock())
        second = SimpleNamespace(startup=AsyncMock(), shutdown=AsyncMock())
        handler.storage = {"a": first, "b": second}
